...
ARXIV_ID_RE = re.compile(r"(?i)(?:arxiv:|10\.48550/arxiv\.)([a-z.-]+/\d{7}|\d{4}\.\d{4,5})(?:v\d+)?")
...
def main(argv: list[str] | None = None) -> int:
    ...
    attachments = _discover_attachments(bindings, source_only=not args.include_non_source_html)
    if only_keys:
        attachments = [attachment for attachment in attachments if attachment.key in only_keys]
    if args.resume:
        attachments = [attachment for attachment in attachments if attachment.key not in completed]
        ...
        try:
            ...
            if sqlite_path not in backed_up_sqlite:
                backup = _sqlite_backup(
                    sqlite_path,
                    run_root / "sqlite" / f"{_safe_name(attachment.binding.library_id)}.sqlite",
                )
            ...
            polished = polish_web_html_file(attachment.host_path, source_url=attachment.source_url)
            ...
            if args.skip_unchanged and before_hash == after_hash:
                record.update({"ok": True, "status": "unchanged", "finished_at": _utc_now()})
                counts["unchanged"] += 1
...
def _discover_attachments(bindings: list[RelayBinding], *, source_only: bool = True) -> list[SourceAttachment]:
...
def _resolve_host_attachment_path(*, storage_dir: Path, key: str, zotero_path: str) -> Path:
    if zotero_path.startswith("storage:"):
        return storage_dir / key / zotero_path.removeprefix("storage:")
    if zotero_path:
        return Path(zotero_path)
    htmls = sorted((storage_dir / key).glob("*.html"), key=lambda path: path.stat().st_mtime, reverse=True)
    if htmls:
        return htmls[0]
...
def _source_url_hint(
    *,
    ...
    for value in (parent_archive_id, parent_archive_location, parent_doi, parent_extra, parent_url):
...
def _host_to_container_path(path: Path, binding: RelayBinding) -> str:
    ...
    try:
        ...
        raise RuntimeError(f"Path is outside relay binding {binding.host_data_dir}: {path}") from exc
...
def _sync_local_storage_metadata(
    *,
    ...
    webdav = relay_result.get("webDav") or {}
    metadata_patch = webdav.get("metadataPatch") or {}
    storage_hash = str(webdav.get("md5") or "").strip()
    storage_mtime = _optional_int(webdav.get("mtime"))
    new_version = _optional_int(
        relay_result.get("newAttachmentVersion")
        or relay_result.get("newVersion")
        or metadata_patch.get("newVersion")
    )
    if not storage_hash or storage_mtime is None or new_version is None:
        return {
            "ok": False,
            "updated": False,
            "reason": "missing_relay_webdav_metadata",
        }
    ...
    try:
        ...
        connection.execute(
            ...
            (new_version, attachment.item_id),
        )
        cache_updated = False
        if before_cache is not None and before_cache["data"]:
            patched = _patched_sync_cache_json(
                str(before_cache["data"]),
